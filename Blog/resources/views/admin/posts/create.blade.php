...
@section('sidebar_admin')
    <div class="d-flex flex-column flex-shrink-0 p-3 text-white bg-dark" style="width: 246px; height: 1300px;">
      <span class="rounded mx-auto d-block">
        ...
        <hr>
        <a href="{{route("admin.posts.index")}}" class="nav-link active badge badge-warning mb-auto" aria-current="page">
            <h2> Статьи</h2>
        </a>
        <hr>
        <div>
...
<br>
    <form enctype="multipart/form-data" method="POST" action="{{isset($post) ? route("admin.posts.update",$post->id) : route("admin.posts.store") }}" >
        ...
        <div class="form-group">
            <input name="title" type="text" class="form-control @error('title') border border-danger @enderror"  placeholder="Название">
        </div>
        @error('title')
        <p class="text-danger">{{$message}}</p>
        @enderror
        <div class="form-group">
            <input name="preview" type="text" class="form-control @error('preview') border border-danger @enderror"  placeholder="Описание">
        </div>
        @error('preview')
        <p class="text-danger">{{$message}}</p>
        @enderror
        <div class="form-group">
            <input name="description" type="text" class="form-control @error('description') border border-danger @enderror"  placeholder="Описание">
        </div>
        @error('description')
        <p class="text-danger">{{$message}}</p>
        @enderror
